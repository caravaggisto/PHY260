...
if __name__ == '__main__':
    ...
    data = {'<x>' : random_walks.x_means,
            '<x^2>': random_walks.x2_means}
    ns_values = random_walks.ns_values
    colors = ['b', 'r']
    labels_xaxis = ['# of steps', '# of steps']
    labels_yaxis = ['Mean x-coordinate', 'Mean square of x-coordinate']
    ...
    for n, title in enumerate( data.keys() ):
        ...
        if add_regression[n]:
            ...
            ax.plot( x, a * x, 'y-' )
            x_annotation, y_annotation = 2.5, 42.5
    ...
    fig.savefig( path_for_new_fig_for_newest_pickle + '.pdf' )
